...
source: https://github.com/umautobots/bidireaction-trajectory-prediction/blob/296a50126cd50a1d4a0395696a0567575c4d4df8/bitrap/utils/scheduler.py#L21
'''
...
def sigmoid_anneal(anneal_kws):
    ...
    return lambda step: start + (finish - start)*torch.sigmoid((torch.tensor(float(step), device=device)) * (1./steps_lo_to_hi)- center_step)

def linear_anneal(anneal_kws):
    device = anneal_kws['device']
    start = torch.tensor(anneal_kws['start'], device=device)
    finish = torch.tensor(anneal_kws['finish'], device=device)
    steps_lo_to_hi = torch.tensor(anneal_kws['steps_lo_to_hi'], device=device, dtype=torch.float)
    return lambda step: finish if step > steps_lo_to_hi else start + (finish - start)*(torch.tensor(float(step), device=device) * (1./steps_lo_to_hi))